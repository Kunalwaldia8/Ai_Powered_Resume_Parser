...
def upload_files():
    try:
        ...
        if jd_file and allowed_jd_file(jd_file.filename):
            ...
            with open(os.path.join(session_jd_folder, 'jd_skills.json'), 'w') as f:
...
def categorize_skills():
    ...
    session_resume_folder = request.form['resume_folder']
    session_jd_folder = request.form['jd_folder']
    ...
    # Parse skills and their selected categories from the form
    num_skills = int(request.form['num_skills'])
    skill_categories = {"required": [], "essential": [], "good_to_have": []}
